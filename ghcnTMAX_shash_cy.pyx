...
def process_day_shash(int i, mds, int n, t1_srs_raw, str cityID, sas_params_df):
    """
    Complete processing for a day including 4-parameter SAS transformation and Jacobian.
    """
    cdef str target_day
    ...
    # Define grid in Celsius (raw scale)
    ...
    # using the SAME parameters for all 5 days as discussed.
    ...
    dict_theo = {'Temp': val_u_celsius, 'Density': res_h_celsius}
    df_theo = pd.DataFrame(dict_theo)
    
    dirs = os.path.join('GHCN-Res/TMAX_SHASH', cityID)
    os.makedirs(dirs, exist_ok=True)
    df_theo.to_csv(os.path.join(dirs, f"{cityID}_{target_day}.csv"), index=False)
    print(f"Day {target_day} processed (SHASH-Cython).")
