"""Run TMIN SHASH processing for a configurable odd local window.
...
def default_parameter_file(window_size, city_id):
    ...
    parameter_dir = f"SAS_4Param_{window_size}DayWindow_Results_TMIN"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="GHCN TMIN SHASH-Cython processing with a configurable local window"
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            'SAS_4Param_<N>DayWindow_Results_TMIN/'
    ...
    parser.add_argument(
        ...
        help=(
            ...
            'Results_SHASH_Cython_TMIN_<N>DayWindow_<cityID>'
        ),
    )
    ...
    try:
        import ghcnTMIN_shash_cy_windowN
    except ImportError as exc:
        parser.error(
            'compiled extension ghcnTMIN_shash_cy_windowN is unavailable; run '
            '`python setupTMIN_shash_windowN.py build_ext --inplace` first '
            f'({exc})'
        )
    ...
    # Keep TMIN on its original Celsius scale. The Cython implementation
    # evaluates the local-minimum event directly.
    df['TMIN'] = df['TMIN']/10
    ...
    output_dir = args.output_dir or (
        f"Results_SHASH_Cython_TMIN_{window_size}DayWindow_{cityID}"
    )
    for i in tqdm(
        range(args.start, args.end),
        desc=f"Processing TMIN {cityID} ({window_size}-day window)",
    ):
        if i >= len(mds): break
        ghcnTMIN_shash_cy_windowN.process_day_shash_tmin(
            i,
            mds,
            n,
            t1_srs_raw,
    ...
    t2 = time.time()
    print(
        f"Batch processing for TMIN {cityID} with a {window_size}-day window "
