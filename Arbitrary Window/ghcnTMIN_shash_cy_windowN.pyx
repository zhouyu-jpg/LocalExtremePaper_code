"""Configurable-window TMIN SHASH density calculations.
...
def h_denom(int n, str target_day, df_z):
    """
    Compute the local-minimum probability using a linear MVN transformation.
    $P(X_0 < X_1, X_0 < X_2, ..., X_0 < X_{2n})$
    where $X_0$ is the target day and $X_i$ are neighboring days.
    """
    ...
    # Transformation matrix D: Y_i = X_target - X_i. The target day is at
    # index n in [i-n, ..., i, ..., i+n], and a local minimum has Y_i < 0.
...
def h_t_z(float z, int n, str target_day, df_z, float denominator):
    ...
    # P(X_window > z | target = z) is evaluated as
    # P(-X_window < -z | target = z).
    p_cond = mvn_cdf(np.full(2 * n, -z), -mu_cond, cov_cond)
    
    return (f_z * p_cond) / denominator

def process_day_shash_tmin(int i, mds, int n, t1_srs_raw, str cityID,
    ...
    if not np.isfinite(res_h_denom) or res_h_denom <= 0:
        raise RuntimeError(
            f"Invalid local-minimum probability for {target_day}: {res_h_denom}"
        )
    ...
    if output_dir is None:
        output_dir = f"Results_SHASH_Cython_TMIN_{window_size}DayWindow_{cityID}"
    import os
    os.makedirs(output_dir, exist_ok=True)
    
    df_theo = pd.DataFrame({'Temp': val_u_celsius, 'Density': res_h_celsius})
    df_theo.to_csv(f"{output_dir}/shash_density_{target_day}.csv", index=False)
    print(f"TMIN day {target_day} processed with a {window_size}-day local window.")
